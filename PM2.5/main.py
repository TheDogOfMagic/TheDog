...
def main():
    ...
    # print(comparison_result.index['good'])

    #  === Step 5. 结果展示 ===
    bar = Bar('对比中国环保部和美国驻华大使馆统计的污染状态')
    bar.add("良好", comparison_result.columns.tolist(), comparison_result.loc['good'].tolist(), is_stack=True, legend_top='bottom', xaxis_interval=0)
    bar.add("轻度", comparison_result.columns.tolist(), comparison_result.loc['light'].tolist(), is_stack=True, legend_top='bottom', xaxis_interval=0)
    bar.add("中度", comparison_result.columns.tolist(), comparison_result.loc['medium'].tolist(), is_stack=True, legend_top='bottom', xaxis_interval=0)
    bar.add("重度", comparison_result.columns.tolist(), comparison_result.loc['heavy'].tolist(), is_stack=True, legend_top='bottom', xaxis_interval=0)

    # 保存结果
    bar.render(os.path.join(config.output_path, 'output.html'))
